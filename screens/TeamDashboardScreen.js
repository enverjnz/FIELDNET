...
import { createTeamDashboardStyles, getGameStatusConfig } from '../theme/teamDashboardStyles';
...
export default function TeamDashboardScreen({ teamId, onBack, onOpenTicker, onOpenLiveTicker, onTeamLeft }) {
  ...
  const styles = useMemo(() => createTeamDashboardStyles(colors), [colors]);
  const statusConfig = useMemo(() => getGameStatusConfig(colors), [colors]);
    ...
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}
        >
          <View style={[styles.actionIcon, { backgroundColor: colors.card, overflow: 'hidden' }]}>
            {team?.avatar_teamlogo ? (
              <Image
                source={{ uri: team.avatar_teamlogo }}
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}
        >
          <View style={[styles.actionIcon, { backgroundColor: colors.signOutBg }]}>
        ...
        >
          <View style={[styles.actionIcon, { backgroundColor: colors.card }]}>
            <Newspaper size={26} color={colors.text} />
          </View>
          <View style={styles.actionText}>
        ...
        {games.length === 0 ? (
          ...
          games.map((game) => {
            ...
            const cfg      = statusConfig[statusNorm] ?? statusConfig.scheduled;
            const opponent = game.away_team_name ?? 'Unbekannt';
            const dateStr  = game.game_date
              ? new Date(game.game_date).toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit', year: 'numeric' })
              : null;
                ...
                <View style={styles.gameMeta}>
                  ...
                  {game.game_code ? (
                    <View style={styles.gameMetaItem}>
                      ...
                      <Text style={[styles.gameMetaText, { fontWeight: '700', color: colors.text }]}>{game.game_code}</Text>
                      <TouchableOpacity
                        onPress={(e) => {
                          e?.stopPropagation?.();
                      ...
                      >
                        {copiedCode === game.game_code
                          ...
                          : <Copy size={11} color={colors.text} />
                        }
                      </TouchableOpacity>
                    </View>
                  ) : null}
